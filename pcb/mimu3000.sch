EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L mimu3000-rescue:R R1
U 1 1 5E29E9BB
P 3950 1800
F 0 "R1" V 4030 1800 50  0000 C CNN
F 1 "110R" V 3950 1800 50  0000 C CNN
F 2 "Resistor_THT:R_Axial_DIN0207_L6.3mm_D2.5mm_P7.62mm_Horizontal" V 3880 1800 50  0001 C CNN
F 3 "" H 3950 1800 50  0001 C CNN
	1    3950 1800
	1    0    0    -1  
$EndComp
$Comp
L Device:CP C1
U 1 1 5E2AE194
P 3950 2100
F 0 "C1" H 4068 2146 50  0000 L CNN
F 1 "47µ" H 4068 2055 50  0000 L CNN
F 2 "Capacitor_THT:CP_Radial_D5.0mm_P2.50mm" H 3988 1950 50  0001 C CNN
F 3 "~" H 3950 2100 50  0001 C CNN
	1    3950 2100
	1    0    0    -1  
$EndComp
Wire Wire Line
	3950 2250 3950 2450
Wire Wire Line
	3950 2450 4000 2450
$Comp
L Device:Battery BT1
U 1 1 5E2BAE3F
P 3050 3100
F 0 "BT1" H 3158 3146 50  0000 L CNN
F 1 "4.5V" H 3158 3055 50  0000 L CNN
F 2 "custom:BatteryHolder_Generic_3xAAA" V 3050 3160 50  0001 C CNN
F 3 "~" V 3050 3160 50  0001 C CNN
	1    3050 3100
	1    0    0    -1  
$EndComp
$Comp
L custom:Conn_TOF SENS1
U 1 1 5E2CBB84
P 6350 3150
F 0 "SENS1" H 6268 2725 50  0000 C CNN
F 1 "Sens" H 6268 2816 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_1x04_P2.00mm_Horizontal" H 6350 3150 50  0001 C CNN
F 3 "~" H 6350 3150 50  0001 C CNN
	1    6350 3150
	-1   0    0    1   
$EndComp
$Comp
L custom:H11F1M U1
U 1 1 5E2A0546
P 4300 2550
F 0 "U1" H 4350 2900 50  0000 L CNN
F 1 "AQY211EH" H 4350 2800 50  0000 L CNN
F 2 "Package_DIP:DIP-4_W7.62mm" H 4250 2550 50  0001 C CNN
F 3 "doc/semi_eng_ge1a_aqy21_e.pdf" H 4250 2550 50  0001 C CNN
	1    4300 2550
	1    0    0    -1  
$EndComp
$Comp
L Connector:XLR3 INPUT1
U 1 1 5E29E533
P 5150 1300
F 0 "INPUT1" H 5150 1550 50  0000 C CNN
F 1 "XLR3" H 5350 1050 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 5150 1300 50  0001 C CNN
F 3 "" H 5150 1300 50  0001 C CNN
	1    5150 1300
	1    0    0    -1  
$EndComp
Wire Wire Line
	5500 1300 5450 1300
Wire Wire Line
	5150 1600 5150 1700
Wire Wire Line
	4800 1300 4850 1300
$Comp
L Device:R_POT RV1
U 1 1 5E2BA12E
P 3700 5450
F 0 "RV1" H 3631 5496 50  0000 R CNN
F 1 "1K_POT" H 3631 5405 50  0000 R CNN
F 2 "custom:Potentiometer_Generic_5mm_Single_Horizontal" H 3700 5450 50  0001 C CNN
F 3 "~" H 3700 5450 50  0001 C CNN
	1    3700 5450
	-1   0    0    1   
$EndComp
$Comp
L Connector:AVR-ISP-6 ISP1
U 1 1 5E2B28C5
P 5250 4050
F 0 "ISP1" H 4921 4146 50  0000 R CNN
F 1 "AVR-ISP-6" H 4921 4055 50  0000 R CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x03_P2.54mm_Vertical" V 5000 4100 50  0001 C CNN
F 3 " ~" H 3975 3500 50  0001 C CNN
	1    5250 4050
	1    0    0    -1  
$EndComp
Wire Wire Line
	5150 4500 5150 4450
Wire Wire Line
	5150 3550 5150 3450
Wire Wire Line
	3700 5300 3700 4850
Wire Wire Line
	8750 5250 9050 5250
Wire Wire Line
	7450 4150 7450 4650
Wire Wire Line
	6250 4650 6250 3950
Wire Wire Line
	6350 4650 6350 3850
Wire Wire Line
	6450 4650 6450 4050
$Comp
L power:+3V0 #PWR01
U 1 1 5E41CB28
P 3050 2400
F 0 "#PWR01" H 3050 2250 50  0001 C CNN
F 1 "+3V0" H 3065 2573 50  0000 C CNN
F 2 "" H 3050 2400 50  0001 C CNN
F 3 "" H 3050 2400 50  0001 C CNN
	1    3050 2400
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR03
U 1 1 5E41D9E2
P 3050 3350
F 0 "#PWR03" H 3050 3100 50  0001 C CNN
F 1 "GND" H 3055 3177 50  0000 C CNN
F 2 "" H 3050 3350 50  0001 C CNN
F 3 "" H 3050 3350 50  0001 C CNN
	1    3050 3350
	1    0    0    -1  
$EndComp
$Comp
L power:+3V0 #PWR05
U 1 1 5E4210BA
P 5150 3450
F 0 "#PWR05" H 5150 3300 50  0001 C CNN
F 1 "+3V0" H 5165 3623 50  0000 C CNN
F 2 "" H 5150 3450 50  0001 C CNN
F 3 "" H 5150 3450 50  0001 C CNN
	1    5150 3450
	1    0    0    -1  
$EndComp
$Comp
L power:+3V0 #PWR09
U 1 1 5E4218B0
P 4300 5000
F 0 "#PWR09" H 4300 4850 50  0001 C CNN
F 1 "+3V0" H 4315 5173 50  0000 C CNN
F 2 "" H 4300 5000 50  0001 C CNN
F 3 "" H 4300 5000 50  0001 C CNN
	1    4300 5000
	1    0    0    -1  
$EndComp
$Comp
L power:+3V0 #PWR08
U 1 1 5E4223E1
P 3700 4850
F 0 "#PWR08" H 3700 4700 50  0001 C CNN
F 1 "+3V0" H 3715 5023 50  0000 C CNN
F 2 "" H 3700 4850 50  0001 C CNN
F 3 "" H 3700 4850 50  0001 C CNN
	1    3700 4850
	1    0    0    -1  
$EndComp
$Comp
L power:+3V0 #PWR02
U 1 1 5E422BFE
P 6650 2700
F 0 "#PWR02" H 6650 2550 50  0001 C CNN
F 1 "+3V0" H 6665 2873 50  0000 C CNN
F 2 "" H 6650 2700 50  0001 C CNN
F 3 "" H 6650 2700 50  0001 C CNN
	1    6650 2700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR07
U 1 1 5E423825
P 5150 4500
F 0 "#PWR07" H 5150 4250 50  0001 C CNN
F 1 "GND" H 5155 4327 50  0000 C CNN
F 2 "" H 5150 4500 50  0001 C CNN
F 3 "" H 5150 4500 50  0001 C CNN
	1    5150 4500
	1    0    0    -1  
$EndComp
Wire Wire Line
	5650 4150 7450 4150
Wire Wire Line
	5650 4050 6450 4050
Wire Wire Line
	5650 3950 6250 3950
Wire Wire Line
	5650 3850 6350 3850
$Comp
L power:GND #PWR010
U 1 1 5E4346AD
P 9050 5250
F 0 "#PWR010" H 9050 5000 50  0001 C CNN
F 1 "GND" H 9055 5077 50  0000 C CNN
F 2 "" H 9050 5250 50  0001 C CNN
F 3 "" H 9050 5250 50  0001 C CNN
	1    9050 5250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR012
U 1 1 5E434B19
P 3700 6000
F 0 "#PWR012" H 3700 5750 50  0001 C CNN
F 1 "GND" H 3705 5827 50  0000 C CNN
F 2 "" H 3700 6000 50  0001 C CNN
F 3 "" H 3700 6000 50  0001 C CNN
	1    3700 6000
	1    0    0    -1  
$EndComp
Wire Wire Line
	6650 2700 6650 2950
$Comp
L power:GND #PWR011
U 1 1 5E4388C2
P 2550 6100
F 0 "#PWR011" H 2550 5850 50  0001 C CNN
F 1 "GND" H 2555 5927 50  0000 C CNN
F 2 "" H 2550 6100 50  0001 C CNN
F 3 "" H 2550 6100 50  0001 C CNN
	1    2550 6100
	1    0    0    -1  
$EndComp
Wire Wire Line
	2550 4900 2550 5250
$Comp
L Device:R R2
U 1 1 5E2D7AE9
P 2550 5400
F 0 "R2" H 2620 5446 50  0000 L CNN
F 1 "165R" H 2620 5355 50  0000 L CNN
F 2 "Resistor_THT:R_Axial_DIN0207_L6.3mm_D2.5mm_P7.62mm_Horizontal" V 2480 5400 50  0001 C CNN
F 3 "~" H 2550 5400 50  0001 C CNN
	1    2550 5400
	1    0    0    -1  
$EndComp
Wire Wire Line
	2550 5850 2550 6100
$Comp
L Device:LED D1
U 1 1 5E2C9905
P 2550 5700
F 0 "D1" V 2450 5850 50  0000 R CNN
F 1 "LED" V 2350 5850 50  0000 R CNN
F 2 "LED_THT:LED_D3.0mm" H 2550 5700 50  0001 C CNN
F 3 "~" H 2550 5700 50  0001 C CNN
	1    2550 5700
	0    -1   -1   0   
$EndComp
Text GLabel 2550 4900 1    50   Input ~ 0
LED
Text GLabel 7050 4000 1    50   Input ~ 0
LED
Wire Wire Line
	7050 4650 7050 4000
Wire Wire Line
	3700 5600 3700 6000
Text GLabel 3300 5450 0    50   Input ~ 0
POT
Wire Wire Line
	3550 5450 3300 5450
Text GLabel 6950 4000 1    50   Input ~ 0
POT
Wire Wire Line
	6950 4650 6950 4000
Text GLabel 3950 1550 1    50   Input ~ 0
MUTE
Wire Wire Line
	3950 1650 3950 1550
Text GLabel 6850 4000 1    50   Input ~ 0
MUTE
Wire Wire Line
	6850 4000 6850 4650
Wire Wire Line
	4000 2650 3950 2650
$Comp
L power:GND #PWR06
U 1 1 5E443817
P 3950 2900
F 0 "#PWR06" H 3950 2650 50  0001 C CNN
F 1 "GND" H 3955 2727 50  0000 C CNN
F 2 "" H 3950 2900 50  0001 C CNN
F 3 "" H 3950 2900 50  0001 C CNN
	1    3950 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	3950 2900 3950 2650
$Comp
L Connector:XLR3 OUTPUT1
U 1 1 5E29E552
P 5150 2100
F 0 "OUTPUT1" H 5150 2350 50  0000 C CNN
F 1 "XLR3" H 5350 1850 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 5150 2100 50  0001 C CNN
F 3 "" H 5150 2100 50  0001 C CNN
	1    5150 2100
	1    0    0    -1  
$EndComp
Wire Wire Line
	5150 1700 5650 1700
Wire Wire Line
	4800 1300 4800 2100
Wire Wire Line
	5500 1300 5500 2100
Wire Wire Line
	4850 2100 4800 2100
Wire Wire Line
	5450 2100 5500 2100
Connection ~ 5500 2100
Wire Wire Line
	5500 2100 5500 2750
Wire Wire Line
	5150 2400 5650 2400
Wire Wire Line
	5650 2400 5650 1700
Wire Wire Line
	3050 3350 3050 3300
Wire Wire Line
	3050 2900 3050 2850
Wire Wire Line
	3050 2450 3050 2400
$Comp
L Switch:SW_DPST_x2 SW1
U 1 1 5E4634A0
P 3050 2650
F 0 "SW1" V 3004 2748 50  0000 L CNN
F 1 "ONOFF" V 3095 2748 50  0000 L CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x02_Pitch2.54mm" H 3050 2650 50  0001 C CNN
F 3 "~" H 3050 2650 50  0001 C CNN
	1    3050 2650
	0    1    1    0   
$EndComp
Wire Wire Line
	5150 2450 5150 2400
Wire Wire Line
	4550 2450 5150 2450
Connection ~ 5150 2400
$Comp
L Device:C C2
U 1 1 5E3D87F9
P 4300 5450
F 0 "C2" H 4415 5496 50  0000 L CNN
F 1 "0.1uF" H 4415 5405 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4338 5300 50  0001 C CNN
F 3 "~" H 4300 5450 50  0001 C CNN
	1    4300 5450
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 5E3DA51E
P 4300 5600
F 0 "#PWR0101" H 4300 5350 50  0001 C CNN
F 1 "GND" H 4305 5427 50  0000 C CNN
F 2 "" H 4300 5600 50  0001 C CNN
F 3 "" H 4300 5600 50  0001 C CNN
	1    4300 5600
	1    0    0    -1  
$EndComp
$Comp
L Connector:Conn_01x05_Male J1
U 1 1 5E3F474D
P 8350 4000
F 0 "J1" H 8322 3932 50  0000 R CNN
F 1 "OPT" H 8322 4023 50  0000 R CNN
F 2 "Connector_PinHeader_1.27mm:PinHeader_1x05_P1.27mm_Vertical" H 8350 4000 50  0001 C CNN
F 3 "~" H 8350 4000 50  0001 C CNN
	1    8350 4000
	-1   0    0    1   
$EndComp
Wire Wire Line
	7950 4650 7950 4200
$Comp
L power:GND #PWR0102
U 1 1 5E3FB13A
P 7650 3950
F 0 "#PWR0102" H 7650 3700 50  0001 C CNN
F 1 "GND" H 7655 3777 50  0000 C CNN
F 2 "" H 7650 3950 50  0001 C CNN
F 3 "" H 7650 3950 50  0001 C CNN
	1    7650 3950
	1    0    0    -1  
$EndComp
Wire Wire Line
	7950 4200 8150 4200
Wire Wire Line
	7850 4100 8150 4100
Wire Wire Line
	7850 4100 7850 4650
Wire Wire Line
	7750 4000 8150 4000
Wire Wire Line
	7750 4000 7750 4650
$Comp
L power:+3V0 #PWR0103
U 1 1 5E40BA3B
P 7650 3750
F 0 "#PWR0103" H 7650 3600 50  0001 C CNN
F 1 "+3V0" H 7665 3923 50  0000 C CNN
F 2 "" H 7650 3750 50  0001 C CNN
F 3 "" H 7650 3750 50  0001 C CNN
	1    7650 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	8150 3900 7650 3900
Wire Wire Line
	7650 3900 7650 3950
Wire Wire Line
	8150 3800 7650 3800
Wire Wire Line
	7650 3800 7650 3750
Text GLabel 7250 4000 1    50   Input ~ 0
SDA
Text GLabel 7350 4000 1    50   Input ~ 0
SCL
Wire Wire Line
	7250 4650 7250 4000
Wire Wire Line
	7350 4000 7350 4650
Text GLabel 6750 3250 2    50   Input ~ 0
SDA
Text GLabel 6750 3150 2    50   Input ~ 0
SCL
$Comp
L power:GND #PWR04
U 1 1 5E434EA3
P 6650 3400
F 0 "#PWR04" H 6650 3150 50  0001 C CNN
F 1 "GND" H 6655 3227 50  0000 C CNN
F 2 "" H 6650 3400 50  0001 C CNN
F 3 "" H 6650 3400 50  0001 C CNN
	1    6650 3400
	1    0    0    -1  
$EndComp
Wire Wire Line
	6550 2950 6650 2950
Wire Wire Line
	6550 3050 6650 3050
Wire Wire Line
	6650 3050 6650 3400
Wire Wire Line
	6550 3250 6750 3250
Wire Wire Line
	6750 3150 6550 3150
$Comp
L Switch:SW_DPDT_x2 SW2
U 1 1 5E4AD00E
P 5050 2900
F 0 "SW2" H 5050 3185 50  0000 C CNN
F 1 "SW_DPDT_x2" H 5050 3094 50  0000 C CNN
F 2 "Jumper:SolderJumper-3_P1.3mm_Bridged12_RoundedPad1.0x1.5mm" H 5050 2900 50  0001 C CNN
F 3 "~" H 5050 2900 50  0001 C CNN
	1    5050 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	4700 2650 4550 2650
Wire Wire Line
	5250 2800 5250 2750
Wire Wire Line
	5250 2750 5500 2750
Connection ~ 5500 2750
$Comp
L Device:C C4
U 1 1 5E4B8635
P 5500 2900
F 0 "C4" H 5615 2946 50  0000 L CNN
F 1 "1000uF" H 5615 2855 50  0000 L CNN
F 2 "Capacitor_Tantalum_SMD:CP_EIA-7343-15_Kemet-W" H 5538 2750 50  0001 C CNN
F 3 "~" H 5500 2900 50  0001 C CNN
	1    5500 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	5500 3050 5250 3050
Wire Wire Line
	5250 3050 5250 3000
Wire Wire Line
	4700 2900 4850 2900
Wire Wire Line
	4700 2650 4700 2900
$Comp
L Device:C C3
U 1 1 5E3E8912
P 4800 5550
F 0 "C3" H 4915 5596 50  0000 L CNN
F 1 "0.1uF" H 4915 5505 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4838 5400 50  0001 C CNN
F 3 "~" H 4800 5550 50  0001 C CNN
	1    4800 5550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 5000 4300 5150
$Comp
L power:GND #PWR?
U 1 1 5E50B04B
P 4800 5700
F 0 "#PWR?" H 4800 5450 50  0001 C CNN
F 1 "GND" H 4805 5527 50  0000 C CNN
F 2 "" H 4800 5700 50  0001 C CNN
F 3 "" H 4800 5700 50  0001 C CNN
	1    4800 5700
	1    0    0    -1  
$EndComp
$Comp
L power:+3V0 #PWR?
U 1 1 5E50B347
P 4800 5100
F 0 "#PWR?" H 4800 4950 50  0001 C CNN
F 1 "+3V0" H 4815 5273 50  0000 C CNN
F 2 "" H 4800 5100 50  0001 C CNN
F 3 "" H 4800 5100 50  0001 C CNN
	1    4800 5100
	1    0    0    -1  
$EndComp
Wire Wire Line
	4800 5400 4800 5250
$Comp
L MCU_Microchip_ATtiny:ATtiny48-AU U2
U 1 1 5E29D77C
P 6950 5250
F 0 "U2" V 6900 5750 50  0000 C CNN
F 1 "ATTINY88AU" H 6950 5250 50  0000 C CNN
F 2 "Package_QFP:TQFP-32_7x7mm_P0.8mm" H 7550 5250 50  0001 C CIN
F 3 "../doc/ATtiny4888" H 6150 5600 50  0001 C CNN
	1    6950 5250
	0    -1   -1   0   
$EndComp
Wire Wire Line
	5150 5250 4800 5250
Connection ~ 4800 5250
Wire Wire Line
	4800 5250 4800 5100
Wire Wire Line
	5150 5150 4300 5150
Connection ~ 4300 5150
Wire Wire Line
	4300 5150 4300 5300
$EndSCHEMATC
