...
// Send a START or repeated start signal
void i2c_start(void)
{
    SDA_HIGH;
    SCL_HIGH;
...
bool i2c_send(uint8_t byte)
{
    ...
    for (int8_t bit = 7; bit >= 0; bit--)
    {
        ...
        SCL_HIGH;
        I2C_DELAY;
        while (!SCL) {}; // for clock stretching
        SCL_LOW;
        byte = (byte << 1);
    }
    // Receive ACK
...
uint8_t i2c_communicate(uint8_t slave_address, uint8_t* send_data, uint8_t send_bytes, uint8_t* rcv_data, uint8_t rcv_bytes)
{
    ...
    // shift the address left to gain space for the r/w flag
    ...
    if (rcv_bytes>0)
    {
        ...
        while (rcv_bytes>0)
        {
            if (rcv_bytes==1)
            {
                // confirm the last byte with NAK
                *rcv_data=i2c_receive(false);
            }
            else
            {
                // confirm all other bytes with ACK
